...
describe('/v1/weights CRUD', () => {
  it('creates and lists a weight', async () => {
    const body = { id: 'w-1', valueKg: 82.5, occurredAt: 1700000000000, updatedAt: 1700000000000 };
    ...
    expect(resList.body.items[0]).toMatchObject({ id: 'w-1', valueKg: 82.5, occurredAt: 1700000000000 });
    expect(typeof resList.body.syncStamp).toBe('number');
  });

  it('updates an existing weight and filters by since', async () => {
    const base = { id: 'w-2', valueKg: 80, occurredAt: 1700000000000, updatedAt: 100 };
    await request(app).post('/v1/weights').send(base);

    const resUpdate = await request(app)
      .put('/v1/weights/w-2')
      .send({ valueKg: 79.2, occurredAt: 1700000001000, updatedAt: 200 });
    ...
    expect(resListSince.body.items[0]).toMatchObject({ id: 'w-2', valueKg: 79.2 });
  });

  it('deletes a weight', async () => {
    const base = { id: 'w-3', valueKg: 90, occurredAt: 1700000000000, updatedAt: 1 };
    await request(app).post('/v1/weights').send(base);
  ...
  it('sanitizes input values', async () => {
    ...
    const raw = { id: 'abc$123', valueKg: -5, occurredAt: -10, updatedAt: -1 };
